...
describe('NatsTransportStrategy', () => {
  ...
  describe('listen', () => {
    it('bootstraps correctly', (complete) => {
      ...
      const subscribeToEventPatternsSpy = jest.spyOn(
        strategy,
        'subscribeToEventPatterns',
      );
      const subscribeToMessagePatternsSpy = jest.spyOn(
        strategy,
        'subscribeToMessagePatterns',
      );

      const loggerSpy = jest.spyOn(strategy['logger'], 'log');

      jest
        .spyOn(strategy, 'createNatsConnection')
      ...
      strategy.listen(() => {
        ...
        expect(createStreamsSpy).toBeCalledWith(
          strategy['jetstreamManager'],
          undefined,
        );
        ...
        expect(subscribeToEventPatternsSpy).toBeCalledWith(
          strategy['jetstreamClient'],
        );

        expect(subscribeToMessagePatternsSpy).toBeCalledTimes(1);
        expect(subscribeToMessagePatternsSpy).toBeCalledWith(
          strategy['connection'],
        );
  ...
  describe('createDurableName', () => {
    it('creates a compliant durable name', () => {
      expect(
        strategy.createDurableName('this > ', 'is', 'a', '*test*'),
